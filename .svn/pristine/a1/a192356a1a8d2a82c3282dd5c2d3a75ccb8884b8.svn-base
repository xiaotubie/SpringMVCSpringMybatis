	...
	<form id="inputForm" action="${ctx}/register" method="post" class="form-horizontal">
		<fieldset>
			...
			<div class="control-group">
				...
				<div class="controls">
					<select id="roles" name="roles" class="required" style="width: 223px;">
						<option value="">-----请选择-----</option>  
						<c:forEach items="${roles}" var="role">	
							<option value="${role.id}" <c:if test="${role.id == user.roles}">selected="selected"</c:if>>${role.roleName}</option>
						</c:forEach>  
					</select>
				</div>
			</div>
